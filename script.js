...
const idleSpriteURL = 'https://raw.githubusercontent.com/Ben00000000/asstes/main/spritesheet%20(11)%20(1).png';
const runSpriteURL = 'https://raw.githubusercontent.com/Ben00000000/asstes/main/spritesheet%20(11)%20(1).png';

const downSpriteURL = 'https://raw.githubusercontent.com/Ben00000000/asstes/main/spritesheet%20(11)%20(1).png';

const upSpriteURL = 'https://raw.githubusercontent.com/Ben00000000/asstes/main/spritesheet%20(11)%20(1).png';
...
function checkBulletCollision(bullet) {
    ...
    document.querySelectorAll('.meteorite').forEach((meteorite) => {
        const meteoriteRect = meteorite.getBoundingClientRect();
        if (isColliding(bulletRect, meteoriteRect)) {
            // Remove the bullet and meteorite
            if (bullet.parentNode) {
...
function decreaseUltimateBossHP() {
    ...
    ultimateBossHP -= 0.5; // Decrease by 10 for demonstration purposes
...
function addUltimateBoss() {
    ...
    if (!hasUltimateBossSpawned) {
        ...
        const circularCenter = { x: 100, y: 300 }; // Set the center of the circular path
...
const meteoriteSpeed = 5; // Adjust as needed
const meteoriteSpawnInterval = 2000; // Adjust as needed (milliseconds)
...
function createMeteorite() {
    ...
    const meteoriteX = Math.random() * (window.innerWidth - meteorite.offsetWidth);
    const meteoriteY = 0;
    meteorite.style.left = `${meteoriteX}px`;
    meteorite.style.top = `${meteoriteY}px`;

    fallMeteorite(meteorite);
}

function fallMeteorite(meteorite) {
    function fall() {
        const meteoriteRect = meteorite.getBoundingClientRect();
        const meteoriteY = meteoriteRect.top + meteoriteSpeed;

        if (meteoriteY > window.innerHeight) {
            // Remove the meteorite when it goes out of the screen
            document.body.removeChild(meteorite);
        } else {
            meteorite.style.top = `${meteoriteY}px`;

            // Check for collisions with player or bullets
            checkMeteoriteCollision(meteorite);
...
function checkMeteoriteCollision(meteorite) {
    const meteoriteRect = meteorite.getBoundingClientRect();
    const playerRect = player.getBoundingClientRect();
...
setInterval(createMeteorite, meteoriteSpawnInterval);
